...
import {GoodsReceiptNoteItemToolbar,GoodsReceiptNoteItemStatistics,GoodsReceiptNoteItemSearch,GoodsReceiptNoteItemTable,GoodsReceiptNoteItemPagination,GoodsReceiptNoteItemModal,GoodsReceiptNoteItemView,
DeleteGoodsReceiptNoteItemDialog} from "./index";
...
const GoodsReceiptNoteItemList = () => {
    ...
    const loadGoodsReceiptNoteItems = async () => {
        try {
            setLoading(true);
            const response = await apiService.getGoodsReceiptNoteItems();
            setItems(
                response.data || []
            );
        }
            ...
            setSnackbar({open:true,message:"Failed to load Goods Receipt Note Items",severity:"error"});
        }
        finally {
            setLoading(false);
        }
    ...
    const filteredItems = useMemo(() => {
        ...
        return items.filter(item =>String(item.GoodsReceiptNoteItemId).includes(search)||String(item.GoodsReceiptNoteId).includes(search)|| String(item.ProductId).includes(search));
    }, [items,searchText]);
...
const statistics = useMemo(() => {
    return {
        ...
        totalRejected:items.reduce((sum, item) =>sum +Number(item.RejectedQuantity || 0),0),
...
>
    ...
    <GoodsReceiptNoteItemStatistics statistics={statistics}    />
    <GoodsReceiptNoteItemSearch searchText={searchText} setSearchText={setSearchText}/>
    {
        loading ?
        (
        ...
        (
            <GoodsReceiptNoteItemTable   items={paginatedItems}  onView={handleView}  onEdit={handleEdit}  onDelete={handleDelete}/>
