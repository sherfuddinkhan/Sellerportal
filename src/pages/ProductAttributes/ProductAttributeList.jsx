...
const ProductAttributeList = () => {
    ...
    const loadAttributes = async () => {
        ...
        try {
            setLoading(true);
            const response = await apiService.getProductAttributes();
...
result = result.filter(item => String(item.ProductId).toLowerCase().includes(search) || item.AttributeName ?.toLowerCase() .includes(search) || item.AttributeValue
                    ?.toLowerCase().includes(search) || item.AttributeType ?.toLowerCase() .includes(search));
        }
        if (attributeTypeFilter !== "") {
            result = result.filter(
                item =>  item.AttributeType === attributeTypeFilter
            );
        }
        setFilteredAttributes(result);
        setPage(1);
    }, [attributes,searchText,attributeTypeFilter]);
    ...
    const pagedAttributes = filteredAttributes.slice((page - 1) * pageSize,page * pageSize);
    // ==========================================
    // Save
    // ==========================================
    ...
    return (
        <Box sx={{ p: 3 }}>
            <ProductAttributeToolbar
                ...
                onExport={() =>
                    console.log("Export Product Attributes")
                }
            />
            ...
            <ProductAttributeSearch
                ...
                setAttributeTypeFilter={setAttributeTypeFilter}
                attributes={attributes}
            />
