...
const ProductPriceSearch = ({
    ...
    const priceTypes = [...new Set(productPrices.map(item => item.PriceType).filter(Boolean))];
    const currencies = [...new Set(productPrices.map(item => item.Currency).filter(Boolean))];
    return (
        <Paper
            elevation={2}
            sx={{
